...
export const getUsers = async (
  ...
  const [userRows] = await pool.query<RowDataPacket[]>(
		`SELECT user.user_id, user.user_name, user.office_id, user.user_icon_id, office.office_name, file.file_name FROM user
		JOIN office ON user.office_id = office.office_id
		JOIN file ON user.user_icon_id = file.file_id
		ORDER BY user.entry_date ASC, user.kana ASC LIMIT ? OFFSET ?`,
		[limit, offset]);
